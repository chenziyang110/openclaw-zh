...
| 环节 | 说明 |
| :--- | :--- |
| **构建** | `pnpm build` → tsdown 打包 ESM 输出至 `dist/`，再生成 Plugin SDK `.d.ts` 类型声明；**Control UI 需单独构建**：`pnpm ui:build` 输出至 `dist/control-ui/`，否则访问 Dashboard 会返回 503 |
...
## 8. 控制面板（Dashboard）无法打开

Gateway 启动后，控制面板（Control UI）通过 **HTTP 与 WebSocket 共用同一端口**（默认 `18789`）提供。

### 访问地址

- **默认（无 basePath）**：浏览器打开 `http://127.0.0.1:18789/`（或 `http://localhost:18789/`）。
- **若配置了 `gateway.controlUi.basePath`**（例如 `"/openclaw"`）：访问 `http://127.0.0.1:18789/openclaw/`。

可通过 `openclaw gateway status` 或 `openclaw status` 查看输出的 **Dashboard** 一行得到当前可用的 URL。

### 常见原因与处理

| 现象 | 原因 | 处理 |
| :--- | :--- | :--- |
| 打开后显示 503，提示 "Control UI assets not found" | 未构建 Control UI 静态资源 | 在仓库根目录执行 `pnpm ui:build`（会安装 UI 依赖并输出到 `dist/control-ui/`），然后重启或刷新 |
| 页面空白或一直转圈 / WebSocket 连不上 | 端口或地址错误、网关未就绪 | 确认 Gateway 已监听 `ws://127.0.0.1:18789`，用 `openclaw gateway status` 确认 RPC probe 为 ok，并用文档中的 Dashboard URL 访问 |
| 反复要求认证或 "device identity required" | 非安全上下文或未配对设备 | 在 HTTPS 或 `localhost` 下打开；或按文档配置 `gateway.controlUi` 的认证/设备策略 |

详细故障排查见官方文档：[Gateway troubleshooting](https://docs.openclaw.ai/gateway/troubleshooting)、[Control UI](https://docs.openclaw.ai/web/control-ui)。
